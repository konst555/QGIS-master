...
bool QgsLayoutItemMapGrid::writeXml( QDomElement &elem, QDomDocument &doc, const QgsReadWriteContext &context ) const
{
  ...
  mapGridElem.setAttribute( QStringLiteral( "gridFrameOffset" ), qgsDoubleToString( mGridFrameOffset ) );
...
bool QgsLayoutItemMapGrid::readXml( const QDomElement &itemElem, const QDomDocument &doc, const QgsReadWriteContext &context )
{
  ...
  mGridFrameOffset = itemElem.attribute( QStringLiteral( "gridFrameOffset" ), QStringLiteral( "0.0" ) ).toDouble();
...
void QgsLayoutItemMapGrid::drawGridFrameZebraBorder( QPainter *p, const QMap< double, double > &borderPos, QgsLayoutItemMapGrid::BorderSide border, double *extension ) const
{
  ...
  if ( extension )
  {
    *extension = mGridFrameOffset + mGridFrameWidth + mGridFramePenThickness / 2.0;
    return;
  }
  ...
  {
//    currentCoord = - (mGridFrameWidth + mGridFrameOffset);
...
//      pos.insert( mMap->rect().width() + mGridFrameWidth, mMap->rect().width() + mGridFrameWidth );
  ...
  for ( ; posIt != pos.constEnd(); ++posIt )
  {
    ...
    if ( border == QgsLayoutItemMapGrid::Left || border == QgsLayoutItemMapGrid::Right )
    {
      ...
      x = ( border == QgsLayoutItemMapGrid::Left ) ? -(mGridFrameWidth + mGridFrameOffset) : mMap->rect().width() + mGridFrameOffset;
      y = currentCoord;
    }
    else //top or bottom
    {
      ...
      x = currentCoord;
      y = ( border == QgsLayoutItemMapGrid::Top ) ? -(mGridFrameWidth + mGridFrameOffset) : mMap->rect().height() + mGridFrameOffset;
    }
  ...
  //draw corners
  width = height = (mGridFrameWidth+mGridFrameOffset) ;
  p->setBrush( QBrush( mGridFrameFillColor1 ) );
  p->drawRect( QRectF( -(mGridFrameWidth+mGridFrameOffset), -(mGridFrameWidth+mGridFrameOffset), width, height ) );
  p->drawRect( QRectF( mMap->rect().width(), -(mGridFrameWidth+mGridFrameOffset), width, height ) );
  p->drawRect( QRectF( -(mGridFrameWidth+mGridFrameOffset), mMap->rect().height(),                  width, height ) );
...
void QgsLayoutItemMapGrid::drawGridFrameTicks( QPainter *p, const QMap< double, double > &borderPos, QgsLayoutItemMapGrid::BorderSide border, double *extension ) const
{
  ...
  if ( extension )
  {
    if ( mGridFrameStyle != QgsLayoutItemMapGrid::InteriorTicks )
      *extension = mGridFrameOffset+mGridFrameWidth;
    return;
  }
  ...
  for ( ; posIt != borderPos.constEnd(); ++posIt )
  {
    if ( border == QgsLayoutItemMapGrid::Left || border == QgsLayoutItemMapGrid::Right )
    {
      ...
      {
        width = mGridFrameWidth;
        x = ( border == QgsLayoutItemMapGrid::Left ) ? 0-mGridFrameOffset : mMap->rect().width() - mGridFrameWidth + mGridFrameOffset;
      }
      ...
      else if ( mGridFrameStyle == QgsLayoutItemMapGrid::InteriorExteriorTicks )
      {
        ...
        x = ( border == QgsLayoutItemMapGrid::Left ) ? - mGridFrameWidth-mGridFrameOffset : mMap->rect().width() - mGridFrameWidth+mGridFrameOffset;
      }
    }
    else //top or bottom
    {
      ...
      {
        height = mGridFrameWidth;
        y = ( border == QgsLayoutItemMapGrid::Top ) ? 0-mGridFrameOffset : mMap->rect().height() - mGridFrameWidth+mGridFrameOffset;
      }
      else if ( mGridFrameStyle == QgsLayoutItemMapGrid::ExteriorTicks )
      {
        height = mGridFrameWidth;
      ...
      else if ( mGridFrameStyle == QgsLayoutItemMapGrid::InteriorExteriorTicks )
      {
        ...
        y = ( border == QgsLayoutItemMapGrid::Top ) ? -mGridFrameWidth-mGridFrameOffset : mMap->rect().height() - mGridFrameWidth+mGridFrameOffset;
      }
    }
...
void QgsLayoutItemMapGrid::drawGridFrameLineBorder( QPainter *p, QgsLayoutItemMapGrid::BorderSide border, double *extension ) const
{
  ...
  if ( extension )
  {
    *extension = mGridFrameOffset + mGridFramePenThickness / 2.0;
    return;
  }
  ...
  switch ( border )
  {
    case QgsLayoutItemMapGrid::Left:
      p->drawLine( QLineF( 0-mGridFrameOffset, 0-mGridFrameOffset, 0-mGridFrameOffset, mMap->rect().height()+mGridFrameOffset ) );
      //corner left-top
      if ( mGridFrameOffset != 0 )
      {
          X1 = 0-mGridFrameOffset+mGridFramePenThickness / 2.0;
          Y1 = 0-mGridFrameOffset+mGridFramePenThickness / 2.0;
          p->drawLine( QLineF( 0, 0, X1, Y1 ) );
      }
      break;
    case QgsLayoutItemMapGrid::Right:
      p->drawLine( QLineF( mMap->rect().width()+mGridFrameOffset, 0-mGridFrameOffset, mMap->rect().width()+mGridFrameOffset, mMap->rect().height()+mGridFrameOffset ) );
      //corner right-bottom
      if ( mGridFrameOffset != 0 )
      {
          X1 = mMap->rect().width()+mGridFrameOffset-mGridFramePenThickness / 2.0 ;
          Y1 = mMap->rect().height()+mGridFrameOffset-mGridFramePenThickness / 2.0 ;
          p->drawLine( QLineF( mMap->rect().width(), mMap->rect().height(), X1, Y1 ) );
      }
      break;
    case QgsLayoutItemMapGrid::Top:
      p->drawLine( QLineF( 0-mGridFrameOffset, 0-mGridFrameOffset, mMap->rect().width()+mGridFrameOffset, 0-mGridFrameOffset ) );
      //corner right-top
      if ( mGridFrameOffset != 0 )
      {
          X1 = mMap->rect().width()+mGridFrameOffset-mGridFramePenThickness / 2.0 ;
    ...
    case QgsLayoutItemMapGrid::Bottom:
      p->drawLine( QLineF( 0-mGridFrameOffset, mMap->rect().height()+mGridFrameOffset, mMap->rect().width()+mGridFrameOffset, mMap->rect().height()+mGridFrameOffset ) );
      //corner left-bottom
      if ( mGridFrameOffset != 0 )
      {
          X1 = 0-mGridFrameOffset+mGridFramePenThickness / 2.0 ;
